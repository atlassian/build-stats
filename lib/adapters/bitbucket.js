...
async function fetchBitbucketPipelines(buildsDir, user, repo, auth, concurrency, since, downloadHook) {
  let pagelen = 100;
  let maxConcurrent = concurrency || 10;
  let lastDownloaded = since;

  if (!lastDownloaded) {
    // First check which files we already have
    let currentlyDownloaded = await fs.readDir(buildsDir);
    lastDownloaded = currentlyDownloaded.filter(file => file.match(/^.+?\.json$/))
  ...
  let limit = pLimit(maxConcurrent); // limits the number of concurrent requests
