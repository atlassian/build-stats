  ...
  describe('fetchBitbucketPipeline', () => {
    it('should write the build info in standard format', async () => {
      ...
      got.mockImplementation(async (url) => {
        ...
        if (url === `${BITBUCKET_API}/${testUser}/${testRepo}/pipelines/?pagelen=100&page=1&sort=created_on`) {
          const path_to_pipelines_response = f.find('bitbucket_pipeline_response.json');
          const pipeline_response = await readFile(path_to_pipelines_response, 'utf-8');
      ...
      const pipelineData = await readFile(path.resolve(tempDir, '1.json'), 'utf-8');
    ...
    it('should call the downloadHook with the builds', async () => {
      ...
      expect(downloadHookMock).toHaveBeenCalledWith(JSON.parse(pipeline_response).values, 1, 101);

      expect(downloadHookMock).toHaveBeenCalledWith(JSON.parse(pipeline_response).values, 2, 101);
    });
  });
});
