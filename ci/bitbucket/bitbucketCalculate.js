...
async function bitbucketCalculate(builds, period = 1, last = 30) {
  builds = builds
    .filter(build => {
    ...
    range =>
      ...
      console.log(
        `Mean build Time: ${chalk.green(
          leftPad((range.SUCCESSFUL.buildDurationMean / 60).toFixed(2), 5, "0")
        )} mins ${getBar(range.SUCCESSFUL.buildDurationMean, t)}`
      )
  );
}

module.exports = bitbucketCalculate;
